    ...
    return {
        ...
        _fethUserSuccess: function () {
            var url = window.location.pathname,
                ...
                blnConsole = (window.name.indexOf(globals.CONSOLE + '_0') > -1) || window.name === '';
            ...
            if (blnConsole && expiredSeconds < 3000 && uriManager.get('extended') !== '1') {
                $('body').html('<h3 style="padding:0 0 0 20px; color:#ac2b37">Console is already open, this window will attempt to close in 10 seconds. If this window does not close, please close it.</h3>');

                messageManager.send({
                    'action': 'MESSAGE-focus-console',
                    'options': {
                        hash: uriManager.getHash()
                    }
                });
                window.close();
                return;
        ...
        _openViewer: function (strViewer) {
            ...
            require([
                    ...
                    'css!vendor/json-tree/json-tree',
