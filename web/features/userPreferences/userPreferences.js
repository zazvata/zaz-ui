    ...
    var userPreferences = {
        ...
        setValues: function (blnApply) {
            ...
            if (blnApply || context.$overlay.find('.instantApply').is(':checked')) {
                context.applyTheme();
                if (window.name.indexOf(settings.globals.CONSOLE + '_0') > -1) {
        ...
        applyTheme: function () {
            ...
            if (window.preferences['PREFERENCE-user-layout']) {
                if (context.settings['PREFERENCE-user-layout'].splitter) {
                    $body.removeClass('no-splitter');
                } else {
                    $body.addClass('no-splitter');
                }
                if (context.settings['PREFERENCE-user-layout'].flip) {
                    $body.removeClass('no-flip');
                } else {
                    $body.addClass('no-flip');
                }
                if (context.settings['PREFERENCE-user-layout'].orient) {
                    $body.removeClass('no-orient');
                } else {
                    $body.addClass('no-orient');
                }
        ...
        applyUserTheme: function () {
            ...
            $.each(userSettings.themes, function (i, theme) {
                if (strTheme === theme.value) {
                    ...
                    context.settings['PREFERENCE-user-layout']['orient'] = (theme.layout.orient) ? 1 : 0;

                    framework.layoutManager.setTab(theme.layout.tab || false);
                    context.settings['PREFERENCE-user-layout']['tab'] = (theme.layout.tab) ? 1 : 0;

                    framework.layoutManager.setToolbar(theme.layout.toolbar || false);
